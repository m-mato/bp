...
\chapter{Byteman}
\textit{Byteman} je nástroj manipulujúci s bajtkódom určný na zjednodušenie 
správania sa Java aplikácií počas načítavania JVM, alebo za behu programu. 
...
pozostávajú z udalosti, podmienky a akcie. Význam pravidla znamená: Keď nastane 
...
\textit{Byteman Agent}, ktorý konfiguruje JVM pre prácu s pravidlami jazyka
používaného týmto nástrojom.
...
\begin{itemize}
\item Najjednoduchšou možnosťou je použiť argument \textit{script:[PATH]}, ktorý
...
pridávať a odoberať pravidlá.
\end{itemize}

\textit{Byteman} je nastavený aby neinjektoval kód do tried JVM. Pri zmene tried
ako napríklad \textit{String a Thread} je preto nutné zmeniť vlastnosť
\textit{system property org.jboss.byteman.transform.all}. Zároveň je nutné
zaistiť, aby bol \textit{Byteman Agent} načítaný (rovnako ako tieto triedy)
defaultným \textit{classloaderom}.

Agenta je možné inštalovať taktiež do už bežaicich aplikácií \footnote{typicky
ide o dlho bežiace aplikácie ako napríklad aplikačný server JBoss} pomocou
skitpu \textit{bminstall.sh}. \textit{Byteman} je následne možné využiť ako
nástroj na kontrolu správania sa programu~\cite{RedHat:Byteman}. 
